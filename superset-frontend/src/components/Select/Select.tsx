...
const Select = ({
  ...
  return (
    <StyledContainer>
      {header}
      {
        mode === 'multiple' && !isPopover ?

          <div className="checkbox-container">
            <Checkbox>全选</Checkbox>
            <Checkbox.Group defaultValue={selectValue} options={selectOptions.map((element) => {
              return {
                label: element.verbose_name ? element.verbose_name : element.label,
                value: element.value
              }
            })} onChange={onChange} />
          </div>
