...
export default function MenuWrapper({ data, ...rest }: MenuProps) {  
  const newMenuData = {
    ...
    menu: data.menu.map(menu => {
      if (menu.name === "Charts") return { ...menu, label: '主题' }
      if (menu.name === "Data") return {
        ...menu, label: "数据", childs: menu.childs?.splice(0, 2).map((child: any) => {
          if (child.name === "Databases") return { ...child, label: '数据库' }
          if (child.name === "Datasets") return { ...child, label: '数据集',}
        })
      }
    })
  };
