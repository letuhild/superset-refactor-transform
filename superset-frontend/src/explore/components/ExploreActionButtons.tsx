...
const ExploreActionButtons = (props: ExploreActionButtonsProps) => {
  ...
  const [queryBtn, setQueryBtn] = useState({});

  useEffect(() => {
    if (queryBtnConfig) {
      setQueryBtn(queryBtnConfig);
    }
  }, [queryBtnConfig]);
  ...
  const doExportExcle = exportChart.bind(this, {
    formData: latestQueryFormData,
    resultType: 'results',
    resultFormat: 'excle',
  });
  ...
  return (
    <>
      <QueryAndSaveBtns //按钮组 执行、保存
        canAdd={queryBtn?.canAdd}
        onQuery={queryBtn?.onQuery}
        onSave={queryBtn?.onSave}
        onStop={queryBtn?.onStop}
        loading={queryBtn?.loading}
        chartIsStale={queryBtn?.chartIsStale}
        errorMessage={queryBtn?.errorMessage}
        datasourceType={queryBtn?.datasourceType}
      />{' '}
      <div
