...
import { runQuery, CtasEnum } from 'src/SqlLab/actions/sqlLab';
...
function ExploreViewContainer(props) {
  ...
  const [detailQuery, setDetailQuery] = useState({ ...props });
  const [perspectiveQuery, setPerspectiveQuery] = useState({ ...props });
  const [dictionaryList, setDictionaryList] = useState({});

  useEffect(() => {
    let dictionary_data = [];// 字典匹配数据
    const tempProps = { ...props };
    if (dictionaryList) {
      if (props.chart.queriesResponse) {
        dictionary_data = props.chart.queriesResponse[0].data.map(column => { // 遍历原有数据列表
          const dictionary_column = { ...column };// 字典匹配列
          Object.keys(dictionaryList).map(key => { // 匹配字典表
            if (dictionary_column[key]) {
              dictionaryList[key].dictionary[0].forEach(element => { // 替换转换值
                if (dictionary_column[key] === element.a_value) {
                  dictionary_column[key] = element.t_value;
                }
              });
            }
          });
        ...
        tempProps.chart.queriesResponse[0].data = dictionary_data;
      }
    }
    switch (activeKey) {
      case 'query':
        setDetailQuery({ ...tempProps });
        break;
      case 'perspective':
        setPerspectiveQuery({ ...tempProps });
        break;
    }
  }, [props.chart]);
  ...
  const getDictionaryList = () => { // 获取字典列表
    const datasourceColumns = [...props.datasource.columns];
    datasourceColumns.map(column => {
      if (column.dictionary) {
        SupersetClient.get({
          endpoint: `/api/v1/dictionary/${column.dictionary}`,
        }).then(({ json = {} }) => {
          const dictionaryListItem = {};
          startQuery(`${json.dictionary[0].transformation_rules}`, res => {
            dictionaryListItem[column.column_name] = {
              ...json.dictionary[0],
              dictionary: [res.data],
            };
            setDictionaryList({ ...dictionaryList, ...dictionaryListItem });
          });
        });
      }
    });
  ...
  function renderErrorMessage() {
    ...
    switch (errorMessages[0][0]) {
      case 'must have a value':
        errorMessages[0][0] = '不能为空';
        break;
    }
  ...
  function renderChartContainer(queryData, tableQuery) {
    return (
      <ExploreChartPanel
        width={width}
        height={height}
  ...
  return (
    <Styles id="explore-container" style={{ height: '100%' }} height={height}>
      <Global
        styles={css`
          .navbar {
            margin-bottom: 0;
      ...
      <div
        ...
        style={activeKey === 'query' ? null : { display: 'none' }}
      >
        {renderChartContainer(queryBtnConfig, detailQuery)} {/* 明细显示面板 */}
      </div>
      <div
        className={cx(
          'main-explore-content',
        ...
        style={activeKey === 'perspective' ? null : { display: 'none' }}
      >
        {renderChartContainer(queryBtnConfig, perspectiveQuery)}
        {/* 透视显示面板 */}
      </div>
    </Styles>
  );
