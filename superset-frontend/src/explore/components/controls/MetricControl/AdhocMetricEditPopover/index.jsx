...
export default class AdhocMetricEditPopover extends React.PureComponent {
  ...
  renderColumnOption(option) {
    const column = { ...option,type_generic:option.type_generic?option.type_generic:0 };
  ...
  render() {
    ...
    const aggregateSelectProps = {
      ...
      placeholder: t('%s aggregates(s)', Object.keys(AGGREGATES_OPTIONS).length),
    ...
    return (
      ...
      >
        <Tabs
          ...
          allowOverflow
        >
          {/* <Tabs.TabPane key={SAVED_TAB_KEY} tab={t('Saved')}>  // 保存tab页
          ...
          >
            ...
            <FormItem label={t('aggregate')}>
              <Select
                options={Object.keys(AGGREGATES_OPTIONS).map(option => ({
                  value: option,
                  label: AGGREGATES_OPTIONS[option],
                  key: option,
                }))}
