...
export function runQuery(query, resolve = () => {}, reject = () => {}) {
  return function (dispatch) {
...
export function queryEditorSetSchemaOptions(queryEditor, options) {
  return {
    type: QUERY_EDITOR_SET_SCHEMA_OPTIONS,
    queryEditor,
    options
  };
}
  ...
  return function (dispatch) {
    ...
    if (isFeatureEnabled(FeatureFlag.SQLLAB_BACKEND_PERSISTENCE)) {
      return SupersetClient.put({
        ...
        postPayload: {
          sql,
          latest_query_id: queryEditor.latestQueryId
        },
      }).catch(() =>
        dispatch(
          addDangerToast(
...
export function queryEditorSetSelectedText(queryEditor, sql) {
  return {
    type: QUERY_EDITOR_SET_SELECTED_TEXT,
    queryEditor,
    sql
  };
}
...
export function createDatasourceStarted() {
  return {
    type: CREATE_DATASOURCE_STARTED
  };
}
